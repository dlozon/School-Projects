...
public class TennisMatch implements TennisMatchInterface
{
   ...
   public TennisMatch ( String idPlayer1, String idPlayer2, int dateYear, int dateMonth, int dateDay, String tournament, String matchScore )
   {
      ...
      this.matchScore = (matchScore+" ").substring(0, matchScore.length() - 1);
   }
   ...
   public int compareTo( TennisMatch m ) 
   {
      int date = m.getDateInt();
   ...
   // This method is just a call for recProcessMatchScore
   public int recProcessMatchScoreCall() 
   {
      currentSetIndex = 0;
   ...
   // Returns the winner of the match as a 1 or a 2
   ...
   // Converts the match to a readable string
   // This string is used to print to the console
   // This string is also used to export to a file
   public String toString()
   {
      return ( "MATCH/" + this.idPlayer1 + "/" + this.idPlayer2 + "/" + getDateInt() + "/" + this.tournament + "/" + this.matchScore );
   }
}